...
2. **`/data/*.json` is the source of truth**, and within it the order is: the
   2026-08-22 walk (door plates and wayfinding signs) beats the third-party
...
7. **modSUTD is not SUTD, and says so.**
...
cron cannot express "first Saturday": when day-of-month and day-of-week are both
set it fires when EITHER matches, so `1-7` with `Sat` runs all week. Each of
those workflows is scheduled `16:00 UTC on Fridays`, which is Saturday 00:00 in
Singapore, and its first step keeps only the runs where the Singapore day is 7
or less. Every other step is guarded on that step's output.
...
- Room search returning every think tank for `tt` is deliberate, and so is
  fuzziness being off for terms of four characters or less: at that length
  everything is within one edit of everything. See the `venue-search` skill.
...
- Venue records are thin on purpose, and `altNames` is the field agents most
  often pad. It is only for a genuinely different name a sign carries
  ("Seminar Room 2A", "Gym"), because it renders on screen as an "aka" line.
  Spelling and abbreviation variants - "Fab Lab" for Fablab, "ESD office"
  spelt out - are search's job: the SYNONYMS table and `queryVariants` in
  `frontend/src/utils/search.ts`, which also indexes the bracket-free form of
  every name. A variant that doesn't resolve is a bug in that file, never a
  missing altName.
- **`landmarks` and `directions` are not authored fields.** `directions` is
  gone, and every hand-written `landmarks` chip went with it: adjacency is what
  the floor plan is for, and "Building 1 Level 5" was the room's location
  printed twice. `sync-data.mjs` writes `landmarks` instead, as
  `["Building 1, Level 5", "Lift Lobby C"]`, the lobby being the nearest
  surveyed lift in the same building on the same floor (177 of 225 venues have
  one). Do not reintroduce prose directions or type a chip into a venue file -
  add the missing lift to the survey instead.
...
- A review must be a **comment** on the mod's discussion, never the discussion
  ...
  review was lost to this.
- `data/term-calendar.json` is **generated**, by
  `tools/scraper/term_calendar.py`, from SUTD's own academic-calendar page and
  data.gov.sg's public holidays. It runs in the monthly `scrape` job. Do not
  hand-edit it, and do not add a rollover step that does: a file a human has to
  remember to update is right until the first year nobody remembers.
  It carries all 15 trimesters of 2026-2030. One entry covers a whole
  trimester because SUTD runs several terms off the same Mondays - the
  September one is Terms 1, 7 and 9 - so `label` names them all and `terms`
  lists the numbers.
  SUTD numbers the recess week as **week 7**, so counting weeks off the term
  start puts "week 7 Friday" inside the break. The teaching weeks are 1-6 and
  8-14, and both review reminders come off this file: the midterm at the end of
  the last teaching week before recess, the other at the end of the last
  teaching week.
- `DEFAULT_TERM_LABEL` in `logic.ts` is **only a fallback**, for before the
  calendar has loaded or a date between terms. `labelFor()` reads the real
  label out of `term-calendar.json`. It never reaches `/data` either way:
  `fold_slots.py` prints the term and never writes it, and room availability
  keys on the venue alone, so a stale label cannot corrupt a contribution.
...
- The Contribute panel was called About until the ask outgrew the description.
  Its panel id is `contribute`; a saved layout still holding `about` is inert,
