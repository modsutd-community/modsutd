...
describe('review reminders ride along with the timetable', () => {
  ...
  // No calendar passed here, so these are the fallback dates. Where the term
  // IS known the midterm moves off recess week - see termReminders.test.ts.
  it('adds exactly two, on a Friday each', () => {
    expect(reminders.map((r) => r.startDate)).toEqual(['2026-10-30', '2026-12-18']);
  });
  ...
  it('survives into the exported file alongside the classes', () => {
    ...
    expect(summaries.filter((s) => s.startsWith('modSUTD'))).toHaveLength(2);
    expect(summaries.some((s) => /midterm review/i.test(s))).toBe(true);
    expect(summaries.some((s) => /end-of-term review/i.test(s))).toBe(true);
