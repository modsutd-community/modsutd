...
// The catalogue re-sorts its results by code, which throws relevance away - so
// a fuzzy hit on a name or a description ranked exactly as well as a real code
// match, and "50.0" listed 02.136 above 50.001.
describe('a typed mod code is answered by codes', () => {
  const mods = [
    { code: '50.001', name: 'Information Systems & Programming', description: '', pillar: 'ISTD', department: 'x', term: '3', credits: 12, schedules: [] },
    { code: '50.004', name: 'Algorithms', description: 'covers 50 topics', pillar: 'ISTD', department: 'x', term: '4', credits: 12, schedules: [] },
    { code: '02.136', name: 'Lyric Poetry', description: 'a course about 50 poems', pillar: 'HASS', department: 'y', term: '5', credits: 12, schedules: [] },
    { code: '02.501', name: 'Theory and Dynamics', description: '', pillar: 'HASS', department: 'y', term: '6', credits: 12, schedules: [] },
  ] as unknown as Parameters<typeof searchMods>[1];

  it('returns only mods whose code starts with what was typed', () => {
    const out = searchMods('50.0', mods).map((m) => m.code);
    expect(out).toEqual(['50.001', '50.004']);
  });

  it('does not let a name or description outrank a code', () => {
    expect(searchMods('50.0', mods).map((m) => m.code)).not.toContain('02.136');
  });

  it('takes the leading zero as optional, since the catalogue prints it', () => {
    expect(searchMods('2.1', mods).map((m) => m.code)).toEqual(['02.136']);
    expect(searchMods('02.1', mods).map((m) => m.code)).toEqual(['02.136']);
  });

  it('falls back to the pillar when nothing starts with the code', () => {
    expect(searchMods('50.9', mods).map((m) => m.code)).toEqual(['50.001', '50.004']);
  });

  it('still searches names for anything that is not a code', () => {
    expect(searchMods('poetry', mods).map((m) => m.code)).toEqual(['02.136']);
  });
});
});
