...
// MyPortal prints the room as "Lecture Theatre 2 (1.203)". The bracketed half
// is the code, and it is the half that matters in a calendar.
const NAMED_ROOM = /^(.*?)\s*\(\s*(\d{1,2}\s*\.\s*\d{3}[\w-]*)\s*\)\s*$/;

/**
 * The LOCATION line: "1.203 (Lecture Theatre 2)".
 *
 * Code first because a calendar's one-line preview truncates from the right,
 * and the code is what a student walks to. Falls back rather than inventing:
 * a venue name carrying no code keeps both, and no name at all is just the
 * code.
 */
export function placeText(venueName: string | undefined, location: string): string {
  if (!venueName) return location;
  const m = NAMED_ROOM.exec(venueName);
  if (m && m[1]) return `${m[2]} (${m[1]})`;
  if (!location || location === venueName) return venueName;
...
function occurrenceKeys(events: TimetableEvent[]): Map<TimetableEvent, Map<string, string>> {
  ...
  for (const e of events) {
    for (const d of datesOf(e)) {
      const k = e.uidKey && datesOf(e).length === 1
        ? e.uidKey
        : `${e.modCode}|${e.type}|${d}`;
  ...
  for (const e of events) {
    ...
    for (const d of datesOf(e)) {
      // uidKey wins, so an event that moves date keeps its identity. Only
      // honoured for a single-date event: sharing one UID across occurrences
      // would collapse a term of classes into one calendar entry.
      const base = e.uidKey && datesOf(e).length === 1
        ? e.uidKey
        : `${e.modCode}|${e.type}|${d}`;
...
export function buildICS(events: TimetableEvent[], opts: ICSOptions = {}): string {
  ...
  const blocks = events.flatMap((e) => {
    ...
    // Code first, name in brackets: "1.203 (Lecture Theatre 2)". The code is
    // what a student walks to and what the room signs carry, and a calendar's
    // one-line preview truncates from the right - so the half that must
    // survive goes first. MyPortal prints it the other way round.
    const where = placeText(e.venueName, e.location);
    // Instructors only. The room already has its own field, and repeating it
    // here put the same string twice in every event.
