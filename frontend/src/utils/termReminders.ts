...
export function buildTermReminderEvents({ termStartISO, origin = 'https://modsutd.tech', cal = null }: ReminderInput): TimetableEvent[] {
  ...
  const mk = (
    ...
    type: '',
    // Keyed on the TERM, not the date. SUTD moves when an eval opens, and a
    // date-keyed reminder would leave the old one sitting in every calendar
    // that already imported it while a second appeared beside it.
    uidKey: `modsutd-reminder|${which.toLowerCase()}|${termStartISO}`,
    day,
    startTime: '17:00',
    endTime: '17:30',
    location: url,
