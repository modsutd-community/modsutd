...
export interface Mod {
  ...
  noBatchChat?: boolean;
  // Who set the flag above, when a script did. Only `gather_cores.py`
...
export interface TimetableEvent {
   ...
   * `modCode|type|date`.
   *
   * A calendar updates an event in place when the UID matches and SEQUENCE has
   * risen, so text can change freely. A DATE cannot: moving the event changes
   * the key, which orphans the old entry and adds a second one. The two term
   * reminders are the case that matters - the eval week moves - so they key on
   * the term instead, and stay one event whatever week they land in.
   *
   * Only for events that emit a single date; a repeating class needs its date
   * in the key to tell its occurrences apart.
   */
  uidKey?: string;
  type: string;
  day: Schedule['day'];
