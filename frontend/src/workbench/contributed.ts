...
export function exportContributed(): Record<string, unknown> {
  return read() as unknown as Record<string, unknown>;
}

/**
 * Replace the store from a synced bundle.
 *
 * Whole-section replace, not a merge: an entry is a claim about one browser's
 * contribution and the build it was made against, and half-merging two of
 * those produces a claim neither browser ever made.
 */
export function importContributed(raw: unknown): void {
  if (!raw || typeof raw !== 'object') return;
  try {
    localStorage.setItem(KEY, JSON.stringify(raw));
    window.dispatchEvent(new Event(CONTRIBUTED_EVENT));
  } catch {
    /* storage refused - the browser simply keeps what it had */
  }
}
