...
export function WorkbenchUiProvider({ children }: { children: ReactNode }) {
  ...
  const [freshmoreMode, setFreshmoreMode] = useState<FreshmoreMode>(saved.freshmoreMode ?? 'classic');
    ...
    const t = setTimeout(() => {
      try {
        localStorage.setItem(
          STORAGE_KEY,
          JSON.stringify({ filter, pillar, term, currentTerm, currentPillar, sortKey, sortDir, selected, selectedRoom, mobileTab, ttMode: ttMode ?? undefined, freshmoreMode, declared }),
        );
      } catch {
        // non-fatal
      }
  ...
  }, [filter, pillar, term, currentTerm, currentPillar, sortKey, sortDir, selected, selectedRoom, mobileTab, ttMode, freshmoreMode, declared]);
