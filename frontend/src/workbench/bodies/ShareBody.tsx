...
const bookmarklet = (origin: string) =>
    `javascript:(()=>{const t=getSelection().toString()||document.body.innerText.slice(0,4000);const m=t.match(/\\b\\d{2}[.]\\d{3}\\b/)||document.body.innerText.match(/\\b\\d{2}[.]\\d{3}\\b/);const p=new URLSearchParams();p.set('text',t);if(m)p.set('mod',m[0]);open('${origin}/share#'+p.toString());})()`;
