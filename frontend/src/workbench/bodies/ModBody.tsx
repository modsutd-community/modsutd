...
function TeleChat({mod}: {mod: Mod}) {
    ...
    // Overlaid schedules count. The relay validates against main, where a
    // contribution auto-commits within a minute, so a chat can be created long
    // before the build that would show the slots to anyone else - and making
    // the person who pasted wait four hours for a button that already works
    // buys nothing.
    ...
    const awaiting =
        chatMod &&
        (mod.localSchedules === true ||
            awaitingDeploy(mod.code, mod.schedules.length > 0));
    ...
    useEffect(() => {
        // `entry` arriving is enough: the branch below renders the real button
        // instead of this one, so there is no pending flag to clear.
    ...
    return (
        <div className={styles.teleRow}>
            ...
            >
                <TelegramIcon />
                {pending
                    ? "setting up the chat - check back in 5 mins!"
                    : "Join the Tele chat!"}
            </button>
        </div>
    );
