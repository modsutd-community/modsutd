...
import { GISCUS_CONFIG, REVIEWS_THEME, giscusWired as giscusIsWired } from '@/config/giscus';
...
function TeleChat({ mod }: { mod: Mod }) {
  ...
  return (
    <div className={styles.teleRow}>
      <button
        ...
        onClick={async () => {
          ...
          try {
            ...
            notify('setting up the chat - the join button appears here in about 2 minutes', 'ok');
          } catch (e) {
            setPending(false);
            notify((e as Error).message);
          }
      ...
      >
        <TelegramIcon />
        {pending ? 'setting up the chat - usually under 2 min…' : 'Join the Tele chat!'}
      </button>
    </div>
  );
}
...
export function ModBody({ code, onPick, onFocusRoom }: Props) {
  ...
  const fulfils = useMemo(() => {
    ...
    return Object.values(allMods).filter((m) => (m.prerequisites ?? []).includes(mod.code));
  }, [allMods, mod]);
  ...
  if (!mod) {
    return (
      <div className={wb.empty}>
        ...
        <span>
          {loading && code ? `dredging the catalogue for ${code}…` :
            code ? `nothing called ${code} in the catalogue.` : 'pick a module from the catalogue.'}
        </span>
      </div>
    );
  }
  ...
  const totalGrade = mod.grading?.components.reduce((s, c) => s + c.percentage, 0) ?? 0;
  ...
  return (
    <div className={`${wb.scroll} ${styles.wrap}`}>
      <div className={styles.headRow}>
        <div>
          <div className={styles.code} data-code={mod.code}>{mod.code}</div>
          <div className={styles.name}>{mod.name}</div>
        </div>
        <span className={styles.pillarStack}>
          {modPillars(mod).map((p) => (
            <span key={p} className={styles.pillarBadge} style={{ color: pillarColor(p), borderColor: pillarColor(p) }}>
              {p}
            </span>
          ))}
        </span>
      ...
      <button
        ...
        data-tip={
          pinnedTerm !== undefined
            ...
            : inPlan ? 'remove from your plan' : `plans it at T${defaultLevel(mod)}`
        }
        onClick={() =>
          dispatch(inPlan ? deselectMod({ mode: freshmoreMode, code: planKey }) : selectMod({ mode: freshmoreMode, code: planKey, level: defaultLevel(mod) }))
        }
      >
        {pinnedTerm !== undefined
      ...
      <div className={wb.statGrid} style={{ marginTop: 18 }}>
        ...
        <div className={wb.statCell}>
          ...
          <div className={wb.statValue} style={{ color: mod.prerequisites?.length ? '#c9a23a' : '#3ca87a' }}>
      ...
      {w && (
        <>
          <div className={wb.eyebrow} style={{ margin: '18px 0 8px' }}>WORKLOAD · {w.total}h/wk</div>
          <div className={styles.workload}>
            {(['lecture', 'tutorial', 'project', 'preparation'] as const).map((k) =>
              w[k] > 0 ? (
                <div key={k} className={styles.workSeg} style={{ flex: w[k] }} data-tip={`${k} ${w[k]}h/wk`}>
                  <span>{w[k]}</span>
                  <span className={styles.workLabel}>{k.slice(0, 4)}</span>
                </div>
              ) : null,
            )}
          </div>
      ...
      {mod.schedules.length > 0 && (
        <>
          <div className={wb.eyebrow} style={{ margin: '18px 0 8px' }}>
            SCHEDULE <span style={{ color: 'rgba(255,176,0,0.6)' }}>· crowdsourced</span>
          </div>
          {mod.schedules.map((s, i) => (
            <div key={i} className={styles.slot}>
              <span className={wb.amber}>{s.day.slice(0, 3)}</span>
              <span className={wb.dim}>
                {s.startTime}–{s.endTime} · {s.type}
                {s.cohort ? ` · cohort ${s.cohort}` : ''}
                {s.instructors.length > 0 ? ` · ${s.instructors.join(' · ')}` : ''}
              </span>
              <button type="button" className={styles.roomLink} onClick={() => onFocusRoom(s.location)}>
                ▽ {s.location}
              </button>
            </div>
          ))}
      ...
      {mod.grading && (
        <>
          <div className={wb.eyebrow} style={{ margin: '18px 0 10px' }}>GRADING · {totalGrade}% accounted</div>
          {mod.grading.components.map((g) => (
            <div key={g.name} className={styles.gradeRow}>
              <div className={styles.gradeMeta}>
                <span data-tip={g.description ? `${g.description.slice(0, 70)}${g.description.length > 70 ? '…' : ''}` : undefined}>{g.name}</span>
                <span style={{ color: '#ffce5a' }}>{g.percentage}%</span>
              </div>
              <div className={styles.gradeBar}>
                <div style={{ width: `${g.percentage}%`, background: color, height: '100%' }} />
              </div>
            </div>
          ))}
          {mod.grading.passingGrade && (
            <p className={wb.faint} style={{ fontSize: 11 }}>passing grade · {mod.grading.passingGrade}</p>
          )}
        </>
      )}

      <div className={wb.eyebrow} style={{ margin: '18px 0 8px' }}>PREREQUISITES</div>
      <div className={styles.treeWrap}>
        <PrerequisiteTree
          modCode={mod.code}
          tree={mod.prereqTree}
          fallback={mod.prerequisites}
          knownMods={Object.fromEntries(Object.values(allMods).map((m) => [m.code, { code: m.code, name: m.name }]))}
          onPick={onPick}
        />
      </div>
      {mod.corequisites?.length ? (
        <p className={wb.dim} style={{ fontSize: 11.5 }}>
          ...
          {mod.corequisites.map((c, i) => (
            <span key={c}>
              <button type="button" className={styles.modLink} onClick={() => onPick(c)}>{c}</button>
      ...
      {fulfils.length > 0 && (
        <>
          <div className={wb.eyebrow} style={{ margin: '18px 0 8px' }}>UNLOCKS · mods that need {mod.code}</div>
          <div className={styles.unlocks}>
            {fulfils.map((m) => (
              <button key={m.code} type="button" className={styles.unlockChip} onClick={() => onPick(m.code)}>
      ...
      <a href={reportUrl} target="_blank" rel="noopener noreferrer" className={styles.report}>
        ⚠ report bad data
      </a>

      <div className={wb.eyebrow} style={{ margin: '20px 0 8px' }}>SHARE A REVIEW · because every experience counts</div>
      <ReviewForm mod={mod} threadUrl={threadUrl} onPosted={() => setReloadKey((n) => n + 1)} />

      <div className={wb.eyebrow} style={{ margin: '20px 0 8px' }}>REVIEWS · from past students</div>
      {giscusWired ? (
        <Giscus
