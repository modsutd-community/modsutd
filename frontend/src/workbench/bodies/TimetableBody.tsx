...
import type {Schedule, TimetableEvent} from "@/types";
        ...
        try {
            ...
            if (isSampleTimetable(text)) {
                ...
                void postSlots({
                    ...
                    if (ok) {
                        // Keep the slots, not just the mod codes: they are what
                        // the mod page and the heatmaps render until the build
                        // ships them. The term end travels too, so the waiting
                        // state does not need term-window.json to know its term.
                        const byMod: Record<string, Schedule[]> = {};
                        for (const s of slots) {
                            (byMod[s.mod] ??= []).push({
                                type: s.type as Schedule["type"],
                                day: s.day as Schedule["day"],
                                startTime: s.start,
                                endTime: s.end,
                                location: s.venue,
                                instructors: [],
