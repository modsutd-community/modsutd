...
function loadFromStorage(): TimetableState {
  try {
    ...
    if (!raw) return { events: [], plans: { classic: emptyPlan(), ay2026: emptyPlan() } };
    ...
    return {
      ...
      plans: parsed.plans
        ? { classic: plan(parsed.plans.classic), ay2026: plan(parsed.plans.ay2026) }
        // pre-split shape: the single plan was built against the classic core
        : { classic: plan(parsed), ay2026: emptyPlan() },
      savedAt: parsed.savedAt,
    };
  } catch {
    return { events: [], plans: { classic: emptyPlan(), ay2026: emptyPlan() } };
  }
}
...
const slice = createSlice({
  ...
  reducers: {
    ...
    importPlans(state, { payload }: PayloadAction<Record<Curriculum, PlanState>>) {
      ...
      state.plans = { classic: plan(payload?.classic), ay2026: plan(payload?.ay2026) };
      persist(state);
    },
  },
});
