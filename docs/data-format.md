...
- `code` - `XX.YYY` (two digits, dot, three). Always.
- `pillar` - `SMT | EPD | ESD | CSD | DAI | ASD | HASS`. There is no "Freshmore" pillar - freshmore subjects belong to SMT (or HASS). A mod can span extra pillars via its official tags (e.g. 03.007 carries ASD + EPD + SMT); `pillar` is the primary.
...
- `prereqTree` - optional, and optional fields are **omitted, never `null`**. Use it only when the prereq logic actually needs AND/OR/N-of:
...
- `directions` - optional and rare: 9 of the 225 have one. It must not restate
  the room's own name, its code, or "Building X, Level Y". The only thing
  directions can say that the header cannot is where the room sits relative to
  **other** rooms, which is what all nine do.
- `landmarks` - chips, 2-5 words each.
