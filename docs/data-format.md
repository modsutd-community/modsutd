...
- `term` - `1` … `10`. Singular. We don't model multiple offerings per year - pick the canonical one. A page that publishes no `Term N` tag gets **8** if it is an elective and **1** if it is tagged `Freshmore Core`: those are the only two shapes the listing produces, and calling both of them term 1 made the freshmore term the dumping ground for the whole elective catalogue. `term_from_tags` in `tools/scraper/gather_mods.py` decides, and `--self-check` pins it. One course overrides it: 99.504's page names term 6 and publishes no tag at all, so its term is pinned in `OFF_SPACE_ADMIT` beside its pillar. Two more are pinned in `GRADUATE_PAGE`, which holds the courses whose own page calls them a master's course, with the words that say it: 02.522 and 02.563 are filed under 8, where every other graduate course in the catalogue sits. Their `Term 2` and `Term 3` tags stay on the record, because those are that programme's terms and the page publishes them. A code alone never decides this: the nine other `02.5` courses name no audience on their pages and keep the term they publish.
