...
The workflow lives at `.github/workflows/scrape.yml`. It is scheduled Fridays 16:30 UTC, which is Saturday 00:30 in Singapore, and its first step keeps only the runs where the Singapore day is 7 or less - cron cannot express "first Saturday" on its own. SUTD data changes about once a term, so monthly is plenty. `workflow_dispatch` is on, so you can run it from the Actions tab without waiting.
