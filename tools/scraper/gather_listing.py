...
DOTTED_CODE_RE = re.compile(r"\d{2}\.\d{3}")
...
def fetch(url: str, *, delay: float, ttl_hours: float) -> str:
    ...
    cached = _http.CACHE_DIR / f"{key}.html"
    fresh = (
        cached.exists()
        and (time.time() - cached.stat().st_mtime) / 3600 < ttl_hours
    )
    if not fresh:
    ...
    return _http.get(url, ttl_hours=ttl_hours)
...
def parse_page(html: str, code: str, slug_suffix: str) -> dict:
    ...
    name = re.sub(r"^\s*\d{2}[.\-]\s?\d{3}\s*", "", title)
