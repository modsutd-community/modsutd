...
def main() -> int:
    ...
    # Two was a guess at "where an OR could hide". A single listed
    # prerequisite can still be wrong - 10.022 carried a 10.007 that
    # SUTD does not list at all - so the floor is a flag, not a rule.
    ap.add_argument("--min-prereqs", type=int, default=2)
    args = ap.parse_args()
    ...
    print(f"{len(todo)} courses list {args.min_prereqs}+ prerequisites", file=sys.stderr)
